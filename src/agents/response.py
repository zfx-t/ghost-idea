...
class ProjectPitch(BaseModel):
    ...
    core_gameplay: str = Field(
        description="核心玩法/功能：用户怎么玩？交互流程是怎样的？"
    )
    tech_justification: str = Field(
        description="为什么用这个新技术：它在其中发挥了什么不可替代的作用？"
    )
...
class MVPRoadmap(BaseModel):
    ...
    biggest_pitfall: str = Field(
        description="开发过程中可能遇到的最大技术坑点及避坑建议"
    )
...
class AgentResponse(BaseModel):
    ...
    brainstorming_ideas: List[BrainstormIdea] = Field(
        description="3个不同维度的初步项目构想", min_items=3, max_items=3
    )

    # 步骤 3
    best_project_name: str = Field(description="最终选定的最优项目的名称")
    best_project_pitch: ProjectPitch = Field(
        description="最优项目的详细 Pitch（电梯演讲）"
    )
...
system_prompt_template = """你是一位顶尖的"极客产品经理（Geek PM）"兼"创意黑客（Creative Technologist）"。你精通各种前沿计算机技术，同时对生活细节、年轻人的痛点、校园场景以及社会热点有着极其敏锐的洞察力。你极度反感无聊的"教学型项目"（如 ToDo List、图书管理系统、普通博客等）。
...
2. <混沌词云碰撞与发散>（Chaos Word Cloud Mashup）：
   - 当前接收到的随机词云标签池为：[{Word_Cloud_Tags}]（注：这是一个不同种类和层级的随机标签集合）。
   - 【执行动作】：
     1. 启动"创意老虎机"模式。请你从上述标签池中，为每一个项目构想**随机抓取 2 到 3 个看似毫无关联的标签**。
     2. 将这项【新技术】的核心超能力，与你抓取到的标签进行硬核碰撞，强行发生化学反应，推导出 3 个切入点完全不同的初步项目构想。
   - 【输出格式要求】（必须清晰展示你的脑洞推导过程）：
     * 💡 构想 1 [融合标签：标签 A + 标签 B] 
       - [项目名称]：一句话描述（用 [新技术] 结合标签场景，解决了什么痛点或制造了什么极具戏剧性的效果，G 点在哪）。
     * 💡 构想 2 [融合标签：标签 C + 标签 D] 
       - [项目名称]：一句话描述（同上，必须换一个视角的标签组合）。
     * 💡 构想 3 [融合标签：标签 E + 标签 F + 标签 G] 
       - [项目名称]：一句话描述（同上，尝试加入极其荒诞或极其硬核的组合）。
...
4. <MVP 开发路线>（Actionable Roadmap）：
   - 为这个项目规划一个周末（48 小时）可以搞定的"最小可行性产品（MVP）"开发路径。
